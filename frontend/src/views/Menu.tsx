...
export const MenuView = ({ 
  userName, setUserName, handleCreateRoom, 
  handleJoinRoom, inputCode, setInputCode, error 
}: MenuViewProps) => {
  return (
    <div className="max-w-md mx-auto bg-slate-800 p-8 rounded-3xl shadow-2xl border border-slate-700 space-y-6 animate-in fade-in zoom-in duration-300">
      <div className="space-y-2">
        <label className="text-xs font-bold text-slate-500 uppercase ml-1">Becenév</label>
        <input
          type="text"
          value={userName}
      ...
      <div className="pt-4 space-y-4">
        <button onClick={handleCreateRoom} className="w-full bg-yellow-600 hover:bg-yellow-500 py-4 rounded-2xl font-bold transition-transform active:scale-95">
          Új szoba indítása
        </button>
        ...
        <div className="flex gap-2">
          ...
          />
          <button onClick={handleJoinRoom} className="w-2/3 bg-slate-700 hover:bg-slate-600 py-4 rounded-2xl font-bold border border-slate-600">
            Belépés
          </button>
        </div>
      </div>
      {error && <p className="text-red-400 text-center text-sm font-medium animate-bounce">{error}</p>}
    </div>
  );
};